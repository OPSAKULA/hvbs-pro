...
/* ── DEFAULT INDICATORS ──
   Auto-enable Volume, Volume Profile (VPVR), VWAP and Delta Volume (CVD)
   every time a chart finishes loading, so the trader never has to turn
   them on by hand. Any indicator can still be toggled off/on manually
   afterwards from the bottom toolbar. */
function activateDefaultIndicators(){
  const DEFAULT_INDICATORS = ['Volume', 'VPVR', 'VWAP', 'CVD'];
  const toolbar = $('bottomIndicatorToolbar');
  if (!toolbar) return;
  DEFAULT_INDICATORS.forEach(name => {
    const btn = Array.from(toolbar.querySelectorAll('.bit-btn'))
      .find(b => b.textContent.trim() === name);
    if (!btn) return;
    if (!btn.classList.contains('active')) btn.classList.add('active');
    if (typeof window.toggleIndicator === 'function') {
      window.toggleIndicator(name, true);
    }
  });
}
...
function initChart() {
  ...
  // Fresh chart instance = fresh indicator state (old series no longer exist)
  if (typeof activeIndicators === 'object') {
    Object.keys(activeIndicators).forEach(k => delete activeIndicators[k]);
  }
  document.querySelectorAll('#bottomIndicatorToolbar .bit-btn.active').forEach(b => b.classList.remove('active'));
...
window.toggleIndicator = function(indName, isActive) {
  ...
  if (indName === 'MA') {
    ...
    S.chart.priceScale('volume').applyOptions({ scaleMargins: { top: 0.82, bottom: 0 } });
    ...
    // Delta Volume / Cumulative Volume Delta — estimates buy vs sell pressure
    // per candle (tick-rule approximation: direction + body-to-range strength)
    // and plots the running cumulative total, like a real exchange CVD panel.
    const cvdArea = S.chart.addAreaSeries({
      lineColor: '#00E676',
      topColor: 'rgba(0, 230, 118, 0.28)',
      bottomColor: 'rgba(0, 230, 118, 0.02)',
      lineWidth: 1.5,
      priceScaleId: 'cvd',
      title: 'CVD'
    });
    S.chart.priceScale('cvd').applyOptions({ scaleMargins: { top: 0.8, bottom: 0 } });

    const cvdData = [];
    let cum = 0;
    for (let i = 0; i < S.chartData.length; i++) {
      const c = S.chartData[i];
      const vol = c.volume || Math.abs(c.close - c.open) * 1000000 + 100;
      const range = (c.high - c.low) || Math.abs(c.close - c.open) || c.close || 1;
      const bodyStrength = Math.min(0.45, (Math.abs(c.close - c.open) / range) * 0.45);
      const buyRatio = c.close >= c.open ? 0.5 + bodyStrength : 0.5 - bodyStrength;
      const buyVol = vol * buyRatio;
      const sellVol = vol - buyVol;
      cum += (buyVol - sellVol);
      cvdData.push({ time: c.time, value: cum });
    }
    cvdArea.setData(cvdData);
    activeIndicators[indName] = cvdArea;
  } else if (indName === 'CCI') {
